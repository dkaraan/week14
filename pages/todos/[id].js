...
const SingleEvent = ({ itemData }) => {
    ...
    const [inputTodo, setInputTodo] = useState(itemData.todo); //changed from name to game
    ...
    const sendData = async () => {
        try {
            console.log("sending!");
            // try to update doc
            const docref = firebase.firestore().collection("Q93v1BenoFRLtRIISFd1Kotd1S33").doc(itemData.id)//.get();
            ...
            if (!doc.empty) {
                docref.update(
                    {
                        todo: inputTodo,
                        data: firebase.firestore.Timestamp.fromDate(new Date(inputDate)),
                    }
                );
